...
AlanSRaskin.ExifViewer.Makers.dumpSonyTagData = function (ifd, output, is_motorola) {
	var tagnum;
	for (var t in ifd) {
		if (t.match(/^x[0-9A-Fa-f]{4}$/)) {
			tagnum = parseInt('0' + t, 16);
			var data = ifd[t];
			output.push(AlanSRaskin.ExifViewer.Makers.getSonyInterpretedTagData(tagnum, data));
		}
	}
}	// dumpSonyTagData()

AlanSRaskin.ExifViewer.Makers.getSonyInterpretedTagData = function (tagnum, data) {
	var output = [];
	switch (tagnum) {
	default:
