...
AlanSRaskin.ExifViewer.Makers.dumpKonicaMinoltaTagData = function (ifd, output, is_motorola) {
	var tagnum;
	for (var t in ifd) {
		if (t.match(/^x[0-9A-Fa-f]{4}$/)) {
			tagnum = parseInt('0' + t, 16);
			var data = ifd[t];
			output.push(AlanSRaskin.ExifViewer.Makers.getKonicaMinoltaInterpretedTagData(tagnum, data));
		}
	}
}	// dumpKonicaMinoltaTagData()

AlanSRaskin.ExifViewer.Makers.getKonicaMinoltaInterpretedTagData = function (tagnum, data) {
